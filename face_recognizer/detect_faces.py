...
def face_detection(image, prototxt, model, min_confidence=0.5):
    ...
    blob = cv2.dnn.blobFromImage(
        resized, scalefactor=1.0, size=(300, 300), mean=(104.0, 177.0, 123.0)
    )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        ...
        default=0.5,
        type=float,
        help="minimum detection confidence to filter weak detection",
    )
    ...
    (boxes, confidences) = face_detection(
        image, args["prototxt"], args["model"], args["confidence"]
    )
